...
validate_boot_measurements_uefi_event_log(uefi_event_logs) := true if {
...
# Generic function to validate kernel cmdline for any platform and algorithm
validate_kernel_cmdline_uefi(uefi_event_logs) := true if {
...
validate_cryptpilot_config(uefi_event_logs) := true if {
	some i
...
validate_cryptpilot_fde(uefi_event_logs) := true if {
	some i
	uefi_event_logs[i].type_name == "EV_EVENT_TAG"
	uefi_event_logs[i].details.unicode_name == "AAEL"
	uefi_event_logs[i].details.data.domain == "cryptpilot.alibabacloud.com"
