...
{{- end }}
{{- define "coco-trustee.names.sgxConfig" -}}
{{- printf "%s-sgx-config" (include "coco-trustee.fullname" . | trunc 51 | trimSuffix "-") | trunc 63 | trimSuffix "-" -}}
{{- end }}
...
{{- printf "%s-postgres" (include "coco-trustee.fullname" . | trunc 54 | trimSuffix "-") | trunc 63 | trimSuffix "-" -}}
{{- end }}
{{- define "coco-trustee.names.postgresInitdb" -}}
{{- printf "%s-postgres-initdb" (include "coco-trustee.fullname" . | trunc 45 | trimSuffix "-") | trunc 63 | trimSuffix "-" -}}
{{- end }}
...
{{/*
True when unified storage is Postgres.
When true, chart should provision/inject POSTGRES_URL dependencies.
*/}}
...
{{/*
True when kbs session storage is Postgres.
*/}}
...
{{/*
True when any workload requires Postgres (unified storage or KBS session storage).
*/}}
...
{{/*
Postgres mode selector. Supported values: internal, external.
*/}}
...
{{/*
True when postgres resources should be provisioned in-cluster.
*/}}
{{- define "coco-trustee.postgres.useBundled" -}}
...
{{/*
Bundled Postgres (when any workload needs Postgres): release-scoped Secret name and dev credentials; not intended for production.
*/}}
...
Postgres Service exists when any workload needs Postgres.
*/}}
...
{{- $pgip := "" }}
{{- if and $pg $pg.spec.clusterIP (ne $pg.spec.clusterIP "None") }}{{ $pgip = $pg.spec.clusterIP }}{{- end }}
{{- $rvip := "" }}
