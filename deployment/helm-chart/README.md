...
Helm chart for [Confidential Containers](https://github.com/confidential-containers) **Trustee** on Kubernetes: **KBS**, **gRPC AS**, and **RVPS**, with optional bundled **PostgreSQL**. KBS is wired to remote **`coco_as_grpc`** Attestation Service.

## Install

**Requirements**: Kubernetes 1.19+, Helm 3. If bundled Postgres is needed (when **`storageBackend.type: Postgres`** or **`sessionStorageType: Postgres`**), the chart uses PVC-backed storage, so your cluster must provide a usable **StorageClass** (or you must bind an existing claim).
...
> [!NOTE] Intel DCAP: `sgx_default_qcnl.conf`
>
> The chart embeds **`files/sgx_default_qcnl.conf`** into a release-scoped ConfigMap (name `<Helm fullname>-sgx-config`, e.g. **`trustee-sgx-config`** when the release is **`trustee`**) and mounts it on the **gRPC Attestation Service** deployment at **`/etc/sgx_default_qcnl.conf`** (see `templates/as-configmap.yaml` and `templates/as-deployment.yaml`).
> This configuration is **only relevant when the Attestation Service verifies Intel TDX/SGX** (Intel DCAP stack reads this path while resolving TDX quotes). It is **not** used for KBS or RVPS, and it is **not** required when you only use non-TDX verifiers (for example sample or non-Intel evidence paths) or AS images built without Intel TDX DCAP support. To use a different collateral endpoint or corporate proxy, replace the contents of **`files/sgx_default_qcnl.conf`** in your chart fork or overlay before `helm install` / `helm upgrade`.
...
When **`storageBackend.type`** or **`sessionStorageType`** is **`Postgres`**, and **`storageBackend.postgres.mode=internal`** (default), the chart deploys bundled Postgres and injects **`POSTGRES_URL`**.
Postgres storage is PVC-backed: by default the chart creates a PVC; set `storageBackend.postgres.internal.persistence.existingClaim` to reuse an existing PVC.
Bundled Postgres credentials default to demo values (`trustee` / `trustee` / `trustee`) and can be overridden with:

- `storageBackend.postgres.internal.user`
- `storageBackend.postgres.internal.password`
- `storageBackend.postgres.internal.database`

### PostgreSQL (external)

This chart supports connecting to an external Postgres instance. Provide a Secret that contains your Postgres URL (or your custom key name), then set `storageBackend.postgres.mode=external`.
...
When `storageBackend.postgres.mode=external`, the chart does **not** create bundled Postgres resources (`Deployment/Service/PVC/Secret/initdb ConfigMap`), even if Postgres is required by `storageBackend.type` or `sessionStorageType`.
...
Default **`values.yaml`** is intentionally small. Fixed on-disk paths for **LocalFs** / **LocalJson** and the bundled Postgres **Service hostname** are defined in **`templates/_helpers.tpl`** (not overridable via values). You can still merge extra keys with `-f` / `--set` (Helm merges arbitrary values).
...
- `sessionStorageType`: Controls only the KBS protocol session store. Supported values are `Memory`, `LocalJson`, `LocalFs`, `Postgres`; when empty, it follows `storageBackend.type`.
  ...
  - `storageBackend.postgres.mode`: Selects Postgres source; supported values are `internal` and `external`. Default is `internal`.
  ...
  - `storageBackend.postgres.internal.user` / `storageBackend.postgres.internal.password` / `storageBackend.postgres.internal.database`: Bundled Postgres credentials/database name used when `storageBackend.postgres.mode=internal`.
  - `storageBackend.postgres.internal.persistence.existingClaim`: Existing PVC name to reuse for bundled Postgres; when non-empty, the chart mounts this claim and does not create a new Postgres PVC.
  - `storageBackend.postgres.internal.persistence.size`: Requested size (for example `8Gi`) for the auto-created bundled Postgres PVC; used only when `storageBackend.postgres.internal.persistence.existingClaim` is empty.
  - `storageBackend.postgres.internal.persistence.storageClass`: StorageClass for the auto-created bundled Postgres PVC; leave empty to use the cluster default StorageClass.
  - `storageBackend.postgres.internal.persistence.accessModes`: Access modes for the auto-created bundled Postgres PVC; default is `["ReadWriteOnce"]`.
  - `storageBackend.postgres.internal.initKvTables`: When `true`, mounts SQL init scripts into `/docker-entrypoint-initdb.d` for first-time DB initialization (bundled Postgres mode only). If not set, defaults to `true`.
  - `storageBackend.postgres.internal.port`: Port exposed by bundled Postgres Service/container (bundled Postgres mode only).
