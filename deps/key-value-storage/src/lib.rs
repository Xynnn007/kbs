...
impl KeyValueStorageStructConfig {
    /// Convert the configuration to a client with the given namespace.
    ///
    /// The namespace is used to interpreted by the concrete backend, e.g.
    /// the PostgreSQL backend will use the namespace as the table name.
    pub async fn to_client_with_namespace(
        &self,
        ...
        match r#type {
            ...
            KeyValueStorageType::LocalJson => {
                ...
                Ok(Arc::new(local_json::LocalJson::new(config.clone(), namespace)?) as _)
            }
            KeyValueStorageType::LocalFs => {
                ...
                Ok(Arc::new(local_fs::LocalFs::new(config.clone(), namespace)?) as _)
            }
